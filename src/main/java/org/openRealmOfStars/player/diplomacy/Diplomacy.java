 ...
 *
 *
 * Diplomacy for player and handling of it. Diplomacy creates Diplomacy Bonus
 * lists for all players except the one who is creating the Diplomacy.
 *
 */
public class Diplomacy {
     ...
     * 
     * @param maxPlayers
     *            Maximum number of players when game is created
     * @param playerIndex
     *            Which player is creating the list
     */
    ...
    /**
     * Get diplomacy list for player index. This can return null if player index
     * is same as who's diplomacy object this is being called or index is out of
     * bounds.
     * 
     * @param index
     *            Player index.
    ...
    public boolean isTradeAlliance(final int index) {
        if (index > -1 && index < diplomacyList.length) {
            return diplomacyList[index].isBonusType(DiplomacyBonusType.IN_TRADE_ALLIANCE);
        }
        return false;
    }
    ...
    public boolean isAlliance(final int index) {
        if (index > -1 && index < diplomacyList.length) {
            return diplomacyList[index].isBonusType(DiplomacyBonusType.IN_ALLIANCE);
        }
        return false;
    }
}
